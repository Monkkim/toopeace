...
assert.match(html, /<section class="tp-results" id="portfolio"/);
assert.match(html, /<section class="tp-journey" id="story"/);
assert.match(html, /<section class="tp-services" id="services"/);
assert.match(html, /class="tp-primary-nav tp-primary-nav--floating"/);

for (const label of ['Portfolio', 'Story', 'Services']) {
  assert.match(html, new RegExp(`class="tp-primary-nav-link"[^>]*>${label}<`));
}

for (const platform of ['instagram', 'youtube', 'blog']) {
  assert.match(html, new RegExp(`class="tp-hero-social"[^>]*data-platform="${platform}"`));
  assert.match(css, new RegExp(`data-platform="${platform}"`));
}

assert.match(css, /\.tp-hero-social\s*\{[\s\S]*?border:\s*0/);
